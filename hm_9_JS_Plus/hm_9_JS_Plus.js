...
usersList.forEach(function(user) {
    ...
    for (let prop in user) {
        if (typeof user[prop] === 'object') {
            ...
            innerBlock.classList.add(`${prop}-block`);

            for (let innerProp in user[prop]) {
                let innerElement = document.createElement('p');
                innerElement.textContent = `${innerProp}: ${user[prop][innerProp]}`;
                innerBlock.appendChild(innerElement);
            }

            userBlock.appendChild(innerBlock);
        } else {
            let element = document.createElement('p');
            element.textContent = `${prop}: ${user[prop]}`;
            userBlock.appendChild(element);
        }
    }
